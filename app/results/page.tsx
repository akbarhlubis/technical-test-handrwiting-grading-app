...
function ResultsContent({ history }: { history: GradingHistory }) {
  return <>
    ...
    <section className="mt-9"><div className="flex items-end justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-[#83948b]">History</p><h2 className="mt-1 font-serif text-3xl">Results over time</h2></div><span className="text-xs font-semibold text-[#83948b]">{history.lessons.length} {history.lessons.length === 1 ? "lesson" : "lessons"}</span></div><div className="mt-5 space-y-8">{history.lessons.map((lesson, index) => <section key={lesson.lessonId ?? "unknown-" + index} aria-labelledby={"lesson-history-" + index}><div className="mb-3"><h3 id={"lesson-history-" + index} className="font-serif text-2xl text-[#29483c]">{lesson.title}</h3><p className="mt-1 text-xs font-semibold uppercase tracking-[0.15em] text-[#83948b]">{lesson.attempts.length} {lesson.attempts.length === 1 ? "test" : "tests"}</p></div><ResultsMatrix words={lesson.words} attempts={lesson.attempts} /></section>)}</div><div className="mt-4 flex items-center justify-end gap-4 text-xs text-[#71847a]"><span className="flex items-center gap-1.5"><i className="h-2.5 w-2.5 rounded-full bg-[#75a47e]" />Correct</span><span className="flex items-center gap-1.5"><i className="h-2.5 w-2.5 rounded-full bg-[#c76b55]" />Needs practice</span><span className="flex items-center gap-1.5"><i className="h-2.5 w-2.5 rounded-full bg-[#9aa8a0]" />Not recorded</span></div></section>
