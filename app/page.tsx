    ...
    <main className="min-h-screen bg-[#f5f3ed] pb-28 text-[#1b3028]">
      <div className="mx-auto max-w-5xl px-5 sm:px-8">
        <header className="flex items-center justify-between py-7 sm:py-9">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[#83948b]">Welcome back,</p>
            <h1 className="mt-1 font-serif text-3xl tracking-tight sm:text-4xl">Sarah</h1>
          </div>
          <button type="button" aria-label="View notifications" className="relative flex h-11 w-11 items-center justify-center rounded-full border border-[#dce3dc] bg-white text-[#476055] shadow-sm transition hover:bg-[#edf2ed]">
            <BellIcon />
            <span className="absolute right-2.5 top-2 h-1.5 w-1.5 rounded-full bg-[#c15c43]" />
          </button>
        </header>

        <section className="flex items-center justify-between rounded-2xl border border-[#dce3dc] bg-white px-4 py-3 shadow-sm sm:px-5">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#8b9b92]">Student</p>
            <p className="mt-1 text-sm font-semibold text-[#274138]">Lucas - Primary 2</p>
          </div>
          <div className="flex gap-1.5" aria-label="Primary level selector">
            {["P1", "P2", "P3", "P4", "P5", "P6"].map((level) => (
              <span key={level} className={`hidden rounded-full px-2.5 py-1 text-[10px] font-semibold sm:inline-block ${level === "P2" ? "bg-[#dce9df] text-[#276047]" : "text-[#a1aea7]"}`}>{level}</span>
            ))}
            <span className="rounded-full bg-[#dce9df] px-2.5 py-1 text-[10px] font-semibold text-[#276047] sm:hidden">P2</span>
          </div>
        </section>
